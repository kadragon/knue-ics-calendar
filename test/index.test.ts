...
describe('Main Worker Handler', () => {
  ...
  describe('fetch handler', () => {
    ...
    it('should return 304 for matching ETag', async () => {
      ...
      const firstRequest = createMockRequest('https://example.com/calendar.ics');
      const firstResponse = await worker.fetch(firstRequest, env);
      const etag = firstResponse.headers.get('etag');
      
      // Second request with If-None-Match
      const secondRequest = createMockRequest('https://example.com/calendar.ics', {
        'if-none-match': etag!
      });
  ...
  describe('scheduled handler', () => {
    ...
    it('should create calendar with correct metadata', async () => {
      ...
      const storedIcs = await kvStore.get('latest');
      expect(storedIcs).toContain('PRODID:--//Github@kadragon//knue-ics-calendar//KO');
